...
Category: **Sacred Oath**, first chosen at **3rd level**.
Subclass feature rows: 3 (choice), 7, 15, 20.

## Subclass list (official, non-UA) — page listed names without explicit slugs; slugs below follow the site's `class:kebab-name` convention (unconfirmed, verify when fetching each subclass page)

- Ancients (PHB) — `paladin:ancients`
- Conquest (XGE) — `paladin:conquest`
- Crown (SCAG) — `paladin:crown`
- Devotion (PHB) — `paladin:devotion` (already in FEATURE_LIB as "Oath of Devotion")
- Glory (MOoT/TCE) — `paladin:glory`
- Redemption (XGE) — `paladin:redemption`
- Vengeance (PHB) — `paladin:vengeance`
- Watchers (TCE) — `paladin:watchers`
